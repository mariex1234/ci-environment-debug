...
# 2. Installation de WineHQ Stable 9.0 (Tes étapes adaptées au Docker)
RUN mkdir -pm755 /etc/apt/keyrings \
    && wget -O - https://dl.winehq.org/wine-builds/winehq.key | gpg --dearmor -o /etc/apt/keyrings/winehq-archive.key \
    && wget -NP /etc/apt/sources.list.d/ https://dl.winehq.org/wine-builds/ubuntu/dists/jammy/winehq-jammy.sources \
    && apt-get update

# 3. Application du Blocage strict (Pinning) pour la version 9.0
RUN echo "Package: winehq-stable\nPin: version 9.0.0.0~jammy-1\nPin-Priority: 1001\n\nPackage: wine-stable*\nPin: version 9.0.0.0~jammy-1\nPin-Priority: 1001" > /etc/apt/preferences.d/wine-pin

# 4. Installation forcée des composants Wine 9.0
...
# 6. Configuration du Bureau (Terminal auto au démarrage)
RUN mkdir -p /root/.config/openbox \
    && echo 'lxterminal &' > /root/.config/openbox/autostart

WORKDIR /root

# 7. Script de lancement qui pré-télécharge MT5
RUN echo '#!/bin/bash\n\
rm -f /tmp/.X1-lock\n\
...
echo "--- WINE 9.0 EST PRET | MT5 EST PRET ---"\n\
